...
# pa_cv <- st_transform(pa_sf, africa_equal_area_crs)
# target_crs <- sf::st_crs(pa_cv)$wkt
# 
# predictors_z_proj <- terra::project(predictors_z, target_crs, method = "bilinear")
# block_size_m <- 500000
# 
# sb <- blockCV::cv_spatial(
#   x = pa_cv,
#   column = "presence",
#   r = predictors_z_proj[[1]],
#   k = n_folds,
#   size = block_size_m,
#   selection = "random",
#   iteration = 100,
...
pa_cv <- st_transform(pa_sf, 4326) # Use WGS84 coordinates to easily extract latitude

# Extract min and max latitude across occurrences/absences
ymin <- min(st_coordinates(pa_cv)[, 2])
ymax <- max(st_coordinates(pa_cv)[, 2])

# Create 5 latitudinal breaks from South to North
lat_breaks <- seq(ymin, ymax, length.out = n_folds + 1)

# Assign each point to a fold based on latitude cut points
pa_cv$fold_id <- cut(
...
# Re-project presence-absence points back to target raster CRS
pa_sf <- st_transform(pa_cv, crs(template))
st_write(pa_sf, pa_points_output, delete_dsn = TRUE, quiet = TRUE)

# Create polygon geometry for the latitudinal block bands for visualization/masking
...
message("Assigned latitudinal folds:")
print(table(pa_sf$fold_id))
...
formula_glm <- as.formula(paste("presence ~", paste(predictor_names, collapse = " + ")))
formula_gam <- as.formula(paste("as.numeric(as.character(presence)) ~", paste(paste0("s(", predictor_names, ", k = 5)"), collapse = " + ")))
...
############################################################
# 9. Model fitting & prediction functions
############################################################
...
fit_models <- function(train_df) {
  ...
  # 1. GLM
  glm_mod <- glm(formula_glm, data = train_df, family = binomial())
  
  # 2. GAM
  gam_mod <- mgcv::gam(formula_gam, data = train_df, family = binomial(), method = "REML")
  
  # 3. Random Forest
  rf_mod <- randomForest::randomForest(x = train_df[, predictor_names], y = train_df$presence, ntree = 500)
  
  # 4. BRT
  gbm_mod <- gbm::gbm(
    formula = presence_num ~ .,
    data = train_df[, c("presence_num", predictor_names)],
  ...
  # 5. SVM
  svm_mod <- kernlab::ksvm(
    x = as.matrix(train_df[, predictor_names]),
    y = train_df$presence,
    type = "C-svc",
    kernel = "rbfdot",
    prob.model = TRUE
  ...
  list(GLM = glm_mod, GAM = gam_mod, RF = rf_mod, BRT = gbm_mod, SVM = svm_mod, XGBoost = xgb_mod)
}

predict_models_df <- function(models, new_df) {
  pred <- list()
  pred$GLM <- predict(models$GLM, newdata = new_df, type = "response")
  pred$GAM <- predict(models$GAM, newdata = new_df, type = "response")
  pred$RF  <- predict(models$RF, newdata = new_df[, predictor_names], type = "prob")[, "1"]
  pred$BRT <- predict(models$BRT, newdata = new_df[, predictor_names], n.trees = 1500, type = "response")
  
  svm_p <- predict(models$SVM, as.matrix(new_df[, predictor_names]), type = "probabilities")
  pred$SVM <- svm_p[, "1"]
...
model_auc_weights <- cv_metrics %>%
  filter(model %in% c("GLM", "GAM", "RF", "BRT", "SVM", "XGBoost")) %>%
  group_by(model) %>%
...
predict_raster_model_chunked <- function(model_name, model_object, predictors_z, output_file) {
  ...
  for (i in seq_len(b$n)) {
    ...
    if (any(valid_idx)) {
      ...
      p_res <- switch(
        model_name,
        "GLM"     = predict(model_object, newdata = sub_df, type = "response"),
        "GAM"     = predict(model_object, newdata = sub_df, type = "response"),
        "RF"      = predict(model_object, newdata = sub_df, type = "prob")[, "1"],
        "BRT"     = predict(model_object, newdata = sub_df, n.trees = 1500, type = "response"),
        "SVM"     = predict(model_object, as.matrix(sub_df), type = "probabilities")[, "1"],
